...
**Status:** In progress — decisions locked, class list TBD
...
| Check | Action |
|---|---|
| Task `BoardID` not in boards.json (orphan) | Overwrite with `DefaultBoardID` |
| Task and its parent have different `BoardID` | Overwrite task's `BoardID` with parent's |
| Subtree depth > 4 | Sever bidirectionally: clear task's `ParentID` → `""`, remove task from parent's `ChildIDs`, reassign task's `BoardID` → `DefaultBoardID` (promotes to root task) |
...
**ErrorCode → exit code:** `Validation`/`Dirty` → 1, `IoError`/`LockFailure` → 2.
...
struct Task {
    ...
    std::string dueDate;           // ISO 8601 string, "" if none
...
Finalized scheme (supersedes provisional §10.1):
...
- **`validation` namespace** — stateless free functions for §9 field rules (`title`, `description`, `priority`, `tags`, `boardName`, `dueDate`). `dueDate` takes a `Clock` so "not past" is testable. Board-name *uniqueness* is relational → lives in `BoardService`.
...
- **Param structs** — `AddTaskParams` (optionals + `priority=5` default), `UpdateTaskParams` (all `std::optional`, `ToDo` status forbidden), `ListFilter`.
...
- **`TaskService`** — holds `Repository&` + `Clock&`; methods: `add`, `update`, `complete` (recurrence clone), `reopen`, `cancel`, `postpone`, `archive`, `discard`, `undo`, `list`. Private `transition(Task&, Status)` consults the static transition table.
